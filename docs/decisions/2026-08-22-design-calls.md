...
*The thirty was narrowed to two minutes by
[the two-minute window](2026-09-03-the-two-minute-window.md), which is where
the reasoning lives; nothing else in this call moved, and the late path is
still open till midnight. The number itself lives in `captureWindow`
(`lib/app_state/capture_flow.dart`), which is the only place it is written.*
