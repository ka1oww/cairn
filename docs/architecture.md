...
(`plan_extraction`) — rounded out with docx, xlsx and csv extractors sharing
one row model — `import_flow.dart` over it, two more platform edges beside
the app-state band (the file picker, and Apple Vision's text recognition),
