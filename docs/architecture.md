...
nothing re-files photos when a plan is saved. Undated days first match on
unique unchanged content, then only unmatched days fall back to position. That
stable pass keeps photographs with Kyoto and Nara when a preceding Tokyo day
is removed. What a day the re-paste *adds* keeps is settled
...
content is new. A wholly rewritten undated day has no stable content identity,
so the remaining position fallback can still carry it in clean and ask
nothing.
...
| **Capture** | built — `CaptureScreen` over `capture_flow.dart`: the call on the day page, the framing screen with the window's own words, the shutter, the breath with as many retakes as you like — every one of them against the same deadline, which a retake never re-opens and never re-closes — the word, and the keep that writes a row, all of it in the house's own tokens and counting the window down to the second (one clock for the whole route, absent once the window has shut); the camera edge below hands up a front frame with every capture event and the review composes the two — the back frame full (`capture-back-frame`) with the front one as a sticker-edged inset at its corner (`capture-front-frame`), both drawn through `photo_frame.dart` — while a live viewfinder and the drawn burning-thread treatment of that countdown are not built; **every exit from the breath discards what it no longer needs through the flow's one `_discard`**: a discarded attempt and a route pop take both frames, turning the day over takes the front one, and the kept photograph is still the back frame alone — storing the front one is a later slice, so no schema, store, sync or upload knows it exists; the pool's close is enforced at the write path (`open()` refuses and `turnTheDayOver` discards the frame) rather than on a button, because the grace's real intake, the import sweep, does not exist yet | app state | Answers the ping. Back camera primary, small front inset; two-minute window; the late path is always open and visibly late. The inset was explicitly after the line (camera-like-bereal decision) and has since been drawn on the review: the source composes nothing, the composition's layout is the screen's, and what stays after the line is *keeping* the front frame — the pool still holds the back frame alone. It is a **route, not a destination**: the day page's one call is the only way in, which is the same reasoning that kept the Pool off the tab bar until it existed. |
...
| **Pure decision cores** (`lib/logic/`) | built — seven residents: the re-paste merge (`repaste_merge.dart`), the decision core of editing a plan after it was accepted; `plan_text.dart`, the plan said back as text the parser can read again; `parsed_areas.dart`, the one mapping from the parser's detailed provenances to the domain's three (`travellerOwn` > `human` > `parser`); `calendar_days.dart`, the one spelling of "n days later" over a date-only value (`calendarPlusDays`, component arithmetic — never `add(Duration(days: n))`, which lands a day early across a DST fall-back); and the tap-to-Maps rules `maps_handoff.dart` (the whole display, tap-offer and URL rule: placeless lines are inert, and so is a line naming several places until the traveller picks one of them; searchable lines use one of three app URLs — wired into the app-state band); `nearest_areas.dart`, the nearest area either side of a stop that has none (nearer first, duplicate dropped) and `addAreaCandidates`, the one candidate order both add-area dialogs offer (that nearest list first, then every other area the plan names, in plan order) — the nearest rule's two surfaces are the day page's "nearest to" search hints and the head of those shared candidates; and `area_edit.dart`, main's phase-1 scaffolding for re-deriving a day's running areas after an edit, still called from nowhere because the frontend resolves areas onto each stop directly instead | `repositories/` value types, `cairn_model`, `itinerary_parser` — no Flutter, no Riverpod, no IO | The providers that call it | A decision worth unit-testing on its own belongs below the providers, not inside one: the merge is a pure function of (saved plan, repasted plan), so it is testable without a database, a widget or a clock. Its rules are written once, in the file and in `AGENTS.md`; screens never reach it, and `mergeRepaste` is called from exactly one place (`PasteFlow._mergeReparse`). |
...
| **Platform glue** | partial. The camera is behind `CameraSource` (`camera_source.dart`), with the real back-then-front pair on a device and a generated stand-in only after a successful empty camera list. Discovery errors refuse visibly. The document picker is behind `FilePickerEdge` (`file_picker_edge.dart`), Apple Vision's text recognition behind `TextRecognitionEdge` (`text_recognition_edge.dart`), and the session credential behind the `cairn/session_vault` iOS Keychain channel. The tap-to-Maps handoff is behind `LinkOpenerEdge` (`link_opener_edge.dart`, over `url_launcher`). Every URL is a keyless https universal link, so it needs no scheme check. Each has a test one beside it. Location and Sign in with Apple are not started | camera, document picker, text recognition, iOS Keychain, url_launcher, location, Sign in with Apple edges | Capture, the paste box's import pill, boot identity, Join, and the Trail/day page tap-to-Maps handoff | The thin controllers that drive dual capture, tag a pinged photo with GPS so it rides rung 1, keep the refresh token in device-only credential storage, run the sign-in flow, and open a Maps app. Kept out of widgets so screens stay platform-blind. |
...
| Node | State | Knows about | What breaks if it changes | Why it exists |
| --- | --- | --- | --- | --- |
| **Drift store** | partial — the itinerary tables (`itinerary_days`, `itinerary_stops`, `itinerary_set_asides`, `itinerary_day_tombstones`), `photos`, the trip itself: `trip_facts` (one row: which trip, what it is called, who started it), `trip_members` (the roster, with no role column and never one) and `trip_invite_codes` (minted and revoked, with no expiry column — a code dies with a trip), `plan_drafts` (one row, the paste box's pending import), and `pending_captures` (one row, the shutter breath); both pending tables are local-only. Schema v15: v2 dropped the scaffold's disposable `trip_drafts` demo, v3 added photos, v4 added the trip's three, v5 gave a pre-mint trip a real uuid, v6 gave every day the merge clock the shared copy is reconciled on and added `sync_states` — the cursor, not the cargo, and the one table here holding no shared fact at all — v7 added `plan_drafts` so an imported plan survives the process before it is accepted, v8 added the photographs' `photo_outbox` (with `photos.file_path` loosened for rows whose bytes live on another phone, `content_type` for the upload to sign, and the pull's cursor on `sync_states`), and v9 added `itinerary_stops.kind` / `area_text` / `area_source` (what a stop is, the area it searches in, and the provenance that decides who outranks whom) and `app_preferences` (one row, this phone's chosen maps app), v10 gave the trip's name its own sync clock (`trip_facts.name_revised_at_utc_iso`), v11 added `itinerary_day_tombstones` and re-queued outbox rows an older client's too-broad refusal classification had left `refused`, v12 added `pending_captures`, rebased persisted photo paths to `frames/...`, and revived rows falsely refused after an absolute container path went stale, and v13 added `itinerary_stops.place_text` / `place_candidates_json` so parser-extracted expressions survive local persistence, v14 added `trip_facts.time_zone` (the destination's IANA zone, nullable — a missing zone stays unknown) and v15 added `itinerary_stops.chosen_place`, the traveller's nullable pick among those candidates, stored alongside the parser's columns so a re-read cannot overwrite it | `cairn_model` (rows typed against the vocabulary), device disk | Repositories; transitively every reactive read in the app | Typed SQLite with real joins and watchable queries — "photos per day" is one query, and its stream is what makes the UI reactive. Choice validated in `learning/riverpod-drift-demo/` (native backend on iOS, not the demo's wasm detour) and recorded in [`docs/decisions/2026-08-25-riverpod-and-drift.md`](decisions/2026-08-25-riverpod-and-drift.md). |
| **Supabase/R2 client adapter** | partial. The shared facts half is built (`lib/storage/remote/`). `SharedFacts` is the interface, and `PostgrestSharedFacts` speaks it over plain HTTP — including `redeemInvite`, the invite redeem with the server's four refusals typed, which no flow calls yet. Auth is built beside it. `SessionSource` is the interface, `GotrueSessions` mints and keeps a GoTrue **anonymous** account, and `KeychainSessionVault` keeps its account id and refresh token as one device-only Keychain item while migrating the legacy plaintext file once. Sign in with Apple is the unbuilt identity route, and the edge functions and R2 are not built | Supabase Auth, Postgres (PostgREST under RLS), iOS Keychain, both edge functions, R2 (presigned PUT/GET) | Repositories. Nothing else in the app may import a Supabase or HTTP symbol | Wraps the session JWT, the RLS-filtered queries, the edge-function calls, and the direct-to-R2 byte transfers behind one interface the repositories consume. The project location is a `--dart-define` override. Both values default to the hosted project's URL and its **publishable** anon key, which are checked in (`lib/storage/remote/shared_facts.dart`), so an ordinary build reaches the backend with nothing passed. No secret is ever checked in. Never commit a `service_role` key or the database password. Refusing before it sends anything remains the behaviour only for a build that overrides the defines to empty (`--dart-define=CAIRN_SUPABASE_URL=`). |
...
| **Camera (dual capture)** | built for the capture event, not for keeping — `BackCameraSource.takeOne()` takes **both** frames, back first and front second, and hands back a `CapturedFrame` carrying `backPath` and `frontPath`; the capture review reads the second one and draws it as the inset, and no row, no byte and no push carries it yet | Capture | Back primary + front inset, taken as a back-then-front sequence — the spike (`learning/dual-camera-spike/`) established that is what "like BeReal" actually means, and true simultaneous capture is explicitly not being built. One camera is opened and disposed at a time, never two; a device with no front camera is a `CameraRefused`, and a refusal after the first shot discards the back file rather than leaving an orphan. The seam's cleanup is per path, and `capture_flow.dart`'s one `_discard` is what asks for it on every exit: a retake, an abandoned capture and a route pop out of the breath take both files, turning the day over takes the front one, and a pop landing mid-keep takes nothing because the keep itself is already an exit that cleans up (`abandon`'s own doc comment holds why) — so no path out leaves an orphan. Keeping the second frame — a row, bytes, an upload — lands after the first release. |
...
10. **Where a trip stands is decided once, and the close is enforced on both
    ...
    before it lets even the starter past. All four server-side gates read one
    function, and since `0016` that function derives the trip's last day from
    `trip_itinerary_days` rather than from the `trips.end_date` snapshot the
...
- **The shared facts' sync is live when a new trip has an explicit destination
  ...
  hosted project — only the permitted paths. The 266 adversarial checks still
