...
///
/// [photos] is the trip's shared pool. It defaults to an **empty in-memory
/// pool**, which is the honest state of this app: nothing writes a photo yet,
/// so the Pool tab opens on its empty state. It has no Drift tables behind it
/// on purpose — the store arrives with the capture slice, which owns the
/// write path, and it is consumed here and nowhere else. Tests seed it to
/// build the pool's other state.
Widget bootstrapApp({
  ...
  return ProviderScope(
    overrides: [
      ...
      photoRepositoryProvider.overrideWithValue(photos ?? InMemoryPhotoPool()),
